...
(function(){
  define(['jquery', 'react', 'prelude-ls', 'backbone', 'io', 'moment', 'd3'], function($, React, Prelude, Backbone, io, moment, d3){
    var ref$, map, filter, slice, lines, any, fold, Str, i, div, tr, td, span, kbd, button, ul, li, a, h1, h2, h3, input, form, table, th, thead, tbody, label, nav, p, ruby, rt, svg, g, path, rect, text, widget;
    ...
    widget.MinimalPlot = React.createClass({
      getDefaultProps: function(){
        return {
          min: 0,
          max: 1,
      ...
      componentWillMount: function(){
        this.angle = d3.scale.linear().domain([this.props.min, this.props.max]).range([-this.props.delta - Math.PI / 2, this.props.delta + Math.PI / 2]);
        this.props.model.on('change', this.modelChanged.bind(this));
        return this.area = d3.svg.area().interpolate("step-after");
      },
      componentWillUnmount: function(){
        return this.props.model.off('change', this.modelChanged.bind(this));
      },
      ...
      renderSvg: function(){
        var parent, w, h, rOut, rIn, svgg, data, dmin, dmax;
        parent = $(this.getDOMNode()).parent();
        w = parent.width();
        h = parent.height();
        rOut = 0.8 * Math.min(w, h / 2);
        ...
        svgg = d3.select(this.refs.plot.getDOMNode()).attr("width", w).attr("height", h).select("g");
        data = this.props.model.get(this.props.plotAttr);
        if (data) {
          ...
          this.x.range([0, w]).domain([dmin, dmax]);
          this.y.range([h, 0]).domain([
          ...
          }.bind(this)).y0(h).y1(function(d){
            return this.y(d.value);
          }.bind(this));
          svgg.select("path").datum(data).attr("class", "area").attr("d", this.area);
        }
        return this;
      },
      componentDidMount: function(){
